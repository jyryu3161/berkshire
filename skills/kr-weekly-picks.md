...
**실전 보유 종목(`"held": true` 표시)은 유니버스·큐 순서와 무관하게 매 배치
최우선으로 끼어든다** — 청산 판단은 verdict 재평가로만 이뤄지므로, 보유 종목은
마지막 분석 56일 초과 시 자동 재분석된다(90일 신호 동결 방지). 보유 표시가 있는
종목은 기존 리포트와 비교해 verdict 변경 여부(계속 보유 vs 청산)를 명시적으로
결론 내릴 것.

**워치리스트(`"watch": true` 표시)**: 직전 판정이 관망/보류였던 종목의 현재가가
보수(bear) 밴드 ×1.05 이하로 내려오면 큐 순서를 기다리지 않고 배치 앞쪽에
최대 2개까지 끼어든다(분석 14일 경과분만). 이 표시가 있는 종목은 "이전엔
비쌌는데 지금 매수선에 왔다"는 맥락이므로, 직전 리포트 대비 무엇이 달라졌는지
(가격만 하락 vs 펀더멘털 악화)를 구분해 매수 전환 여부를 판정할 것.
