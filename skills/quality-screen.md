...
> **한국(코스피/코스닥) 종목**: 시세·PER/PBR·연간 재무는 네이버금융 `python3 tools/krx_data.py {search|quote|valuation|financials} {코드}`(무키), 원천 재무제표는 DART `python3 tools/dart_data.py {corpcode|financials} ...`(무료키)로 취득. `skills/financial-data.md` 규범(2개 독립 소스 교차검증, 오차>1% 표기) 준수.
